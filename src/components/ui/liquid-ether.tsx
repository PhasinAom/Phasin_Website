...
export default function LiquidEther({
  ...
  useEffect(() => {
    ...
    class MouseClass {
      ...
      private isPointInside(x: number, y: number) {
    ...
    class ShaderPass {
      ...
      constructor(props: Record<string, unknown>) {
        this.props = props || {}; this.uniforms = (this.props.material as Record<string, unknown>)?.uniforms as Uniforms;
      }
      init() {
    ...
    class Advection extends ShaderPass {
      ...
      constructor(simProps: Record<string, unknown>) {
        ...
        this.uniforms = (this.props.material as Record<string, unknown>).uniforms as Uniforms; this.init();
      }
      init() {
        super.init();
    ...
    class ExternalForce extends ShaderPass {
      ...
      update(props: Record<string, unknown> = {}) {
        const mf = (props.mouse_force as number) || 0; const cs = (props.cellScale as THREE.Vector2) || { x:1, y:1 };
        const cur = (props.cursor_size as number) || 0;
        const cx = Math.min(Math.max(Mouse.coords.x, -1+cur*cs.x+cs.x*2), 1-cur*cs.x-cs.x*2);
        const cy = Math.min(Math.max(Mouse.coords.y, -1+cur*cs.y+cs.y*2), 1-cur*cs.y-cs.y*2);
        const u = (this.mouse.material as THREE.RawShaderMaterial).uniforms;
        u.force.value.set((Mouse.diff.x/2)*mf, (Mouse.diff.y/2)*mf);
        u.center.value.set(cx, cy); u.scale.value.set(cur, cur); super.update();
      }
    }
    ...
    class Divergence extends ShaderPass {
      ...
      update(args: { vel?: THREE.WebGLRenderTarget } = {}) {
        if (this.uniforms && args.vel) this.uniforms.velocity.value = args.vel.texture; super.update();
      }
    }
    ...
    class Simulation {
      ...
      createShaderPass() {
        const o = this.options; const f = this.fbos;
      ...
      update() {
        ...
        let vel: THREE.WebGLRenderTarget = f.vel_1!;
        if (o.isViscous) vel = this.viscous.update({ viscous:o.viscous,iterations:o.iterations_viscous,dt:o.dt }) as THREE.WebGLRenderTarget;
