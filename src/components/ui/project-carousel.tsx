...
export function ProjectCarousel() {
  ...
  useGSAP(
    () => {
      ...
      // Check synchronously at run time — avoids isMobile state race condition
  ...
  return (
    <>
      ...
      >
        ...
        <div className="absolute inset-0 flex flex-row gap-12 px-[4vw] pt-20 pb-14">
          <div className="relative w-[42%] flex-shrink-0 flex items-center z-10 h-full">
            {projects.map((p, i) => (
              <div
                key={p.id}
              ...
              >
                ...
                >
                  <p className="text-lg leading-relaxed max-w-xs" style={{ color: `${p.accent}55` }}>
                    {p.description}
                  </p>
                  <div className="flex flex-wrap gap-2">
                    {p.role.map(r => (
